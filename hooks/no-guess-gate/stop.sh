...
if [ "$nask" -gt 0 ]; then log - "(질문 면제)"; exit 0; fi
...
if printf '%s' "$last" | grep -qiE "$ASKRE"; then log - "(질문문 면제)"; rm -f "$s/blocked_at"; exit 0; fi
...
R1="($PATHRE|there('s| is| are) (no|a|an)|(does|doesn't|do not|don't) (contain|exist|have)|존재하지 않|파일이 없|파일이 있|디렉터리에 (없|있))"
...
[ "$ntools" -eq 0 ] && printf '%s' "$last" | grep -qiE "$R1" && v="$v R1"
printf '%s' "$last" | grep -qiE "$R2a" && v="$v R2a"
...
v="${v# }"; log "$ctx" "$v"; if [ -z "$v" ]; then rm -f "$s/blocked_at"; exit 0; fi
echo "$ntools" > "$s/blocked_at"
{
  ...
  case " $v " in *" R2a "*) echo "- R2a: '확인이 필요하다'류의 유보 표현으로 끝냈다. 확인이 필요하면 지금 확인하라. 말만 하고 끝내는 것은 금지다.";; esac
