...
- `tests/done-gate/unit.sh` — 완료 게이트 78건. PR 본문 근거 21건 포함.
- `tests/test-integrity/unit.sh` — 테스트 무결성 65건.
- `tests/project-guard/unit.sh` — 프로젝트 가드 41건.
...
- `tests/invariants.sh` — 매니페스트·CHANGELOG·문서의 숫자, 셸 인용, 하네스의 카탈로그 복사, 메시지 키 커버리지 17건.  **합계 409건.**
